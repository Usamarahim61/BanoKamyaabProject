...
function ManageICS() {
    ...
    const togglePopup1 = () => {
      ...
      const callDegree = async () => {
        try {
          ...
          const res = await fetch('/ViewICSDegree', {
            method: "GET",
            headers: {
  ...
  return (
    ...
    <div className=" mt-5 ">
        <div class="Head">
          <div class="overlay">
            <h1 className="fw-bold fs-1 text-uppercase">Manage ICS Degree</h1>
          </div>
        </div>
      </div>
      <div className="container mt-5 py-5 shadow-lg ">
        ...
        <div className="d-flex justify-content-end mb-5">
          <Link className="btn btn-success py-2" to={"/AddICSDegree"}><i class="bi bi-plus fs-6"></i> Add Degree</Link>
                ...
                <><li className="d-flex border rounded-2 shadow-sm mt-3 px-4 py-2">
                  <table>
                    <tr className="col-9">
                      ...
                      <td className=" col-2 text-end"><Link to={`/UpdateICSDegree/${element._id}`} className="mx-4"><i class="bi bi-pencil-square fs-5 " ></i></Link>
