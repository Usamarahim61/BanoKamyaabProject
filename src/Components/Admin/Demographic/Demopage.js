...
function Demopage() {
  ...
  const [DisplayDemo, setDisplayDemo] = useState([]);
  ...
  useEffect(() => {
    ...
    const DeleteDemographic = async (id) => {
        ...
        const res2 = await fetch(`/DeleteDemographic/${id}`, {
          method: "DELETE",
          headers: {
            // Accept:"application/json",
            "Content-Type": "application/json"
    ...
    <div className="container mt-5 py-5 shadow-lg ">
        <div>
            <div className="d-flex justify-content-end mb-5">
                <Link className="btn btn-success py-2" to={"/AddDemographic"}><i class="bi bi-plus fs-6"></i> Add Questions</Link>

            </div>
            <ol>
                {
                    DisplayDemo.map((q,id)=>(
                <li key={id} className="d-flex border rounded-2 justify-content-between shadow-sm mt-3 px-4 py-2">
                    ...
                    <div className="d-flex justify-content-end col-1">
                        <Link to={`/UpdateDemographic/${q._id}`} ><i class="bi bi-pencil-square fs-5 "></i></Link>
                        <Link className="ms-4" onClick={() => DeleteDemographic(q._id)}><i class="bi bi-trash fs-5" ></i></Link>
                    </div>
